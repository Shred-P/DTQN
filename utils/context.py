...
class Context:
    ...
    def reset(self, obs: np.ndarray):
        ...
        self.action = RNG.rng.integers(self.num_actions, size=(self.max_length, len(self.num_actions)))
    ...
    def roll(self, arr: np.ndarray) -> np.ndarray:
        r"""
        Utility function to help with insertions at the end of the array. If the context is full,
        we replace the first element with the new element, then 'roll' the new element to the end of the array
        """
