...
class DTQN(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if action_dim > 0:
            raise NotImplementedError("动作嵌入还未实现")
            self.action_embedding = ActionEmbeddingRepresentation(
                num_actions=sum(num_actions), action_dim=action_dim
            )
        else:
    ...
    def forward(
        ...
        if self.bag_size > 0:
            ...
            working_memory = torch.concat([working_memory, persistent_memory], dim=-1)

        # Use FFN layers to get Q-values for each action dimension separately
        output = []
        for ffn_layer in self.ffn:
            output.append(ffn_layer(working_memory))
        output = torch.stack(output, dim=-2)  # [batch, seq_len, num_actions, action_dim]
