    ...
    while timestep < prepop_steps:
        ...
        while not done:
            if isinstance(env.action_space, gym.spaces.MultiDiscrete):
                num_actions_per_dim = env.action_space.nvec  # 获取每个维度的动作数量
            else:
                num_actions = env.action_space.n  # 适用于 Discrete 动作空间
            action = np.array([RNG.rng.integers(n) for n in num_actions_per_dim])
            next_obs, reward, terminated, truncated, info = env.step(action)
